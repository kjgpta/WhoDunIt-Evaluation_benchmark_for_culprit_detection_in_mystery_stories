...
def getResults(stories):
    ...
    # Load stories from books.json in meta folder
    with open(os.path.join('meta', 'books.json'), 'r') as books_file:
        stories2 = json.load(books_file)

    # Remove stories present in books.json
    for batch_ref in list(batch_ids.keys()):
        story_name = batch_ref.split('_')[1]
        if story_name in stories2:
            del batch_ids[batch_ref]
...
def main():
    ...
    with open(os.path.join("meta", "done_ss.json"), 'w') as file:
