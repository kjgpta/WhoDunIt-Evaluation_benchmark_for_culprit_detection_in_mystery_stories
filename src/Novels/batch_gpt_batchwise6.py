...
def batching(stories, prompt_styles, models, output_batch_file, start_index=0, batch_size=9):
    batch_ids = {}
    batch_dir = 'batch'
    end_index = min(start_index + batch_size, len(stories))
    current_batch = stories[start_index:end_index]
    print("stories", current_batch)
    batch_path = os.path.join(batch_dir, 'batch_ids.json')
