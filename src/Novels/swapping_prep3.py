...
def main():
    ...
    # Load books.json from meta folder
    with open(os.path.join(meta_dir, 'books.json'), 'r') as books_file:
        books = json.load(books_file)

    # Load killer.json from meta folder
    with open(os.path.join(meta_dir, 'killer.json'), 'r') as killer_file:
        killer_info = json.load(killer_file)

    # Iterate over each story in books.json
    for folder_name, _ in books.items():
